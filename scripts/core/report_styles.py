...
def set_sheet_title(ws, display_name: str, inventory_date: str, center: Alignment) -> None:
    """为每个工作表写入统一标题行。"""
    ws.insert_rows(1)
...
def resolve_preferred_width(header: object) -> float | None:
    """按固定列名或前缀规则解析列宽。"""
...
def resolve_number_format(header: object) -> str | None:
    """按固定列名或前缀规则解析数字格式。"""
    header_text = str(header)
...
def set_freeze_and_filter(ws, sheet_name: str) -> None:
    """设置冻结窗格与筛选范围。"""
    if sheet_name == "使用说明":
        ws.freeze_panes = "A3"
    else:
...
def estimate_row_height(ws, row_idx: int, wrap_columns: set[str], min_height: int = 20) -> float:
    """估算自动换行后的行高。"""
    max_lines = 1
    ...
    for row in ws.iter_rows(min_row=3, max_row=ws.max_row):
        for col_idx, cell in enumerate(row, start=1):
            ...
            if header in wrap_columns:
                ...
                cell.alignment = left
            resolved_number_format = resolve_number_format(header)
        ...
        ws.row_dimensions[row[0].row].height = estimate_row_height(ws, row[0].row, wrap_columns)
    return header_to_idx


def apply_overview_metric_format(ws, header_to_idx: Dict[str, int]) -> None:
    """统一运行总览核心指标的数字格式。"""
...
def merge_detail_store_cells(ws_detail, center: Alignment) -> None:
    """按连续门店名称合并明细页首列，减少视觉噪音。"""
