...
def _build_usage_guide_frame() -> pd.DataFrame:
    rows = [
        ...
        ["门店销量排名调货汇总", "该表以本次运行实际参与计算的销售文件为范围，先汇总门店销售额并按销售额降序排名，再列出有调货动作的商品。门店销售额总计表示该门店在本次销售范围内的总销售额；商品销售额表示该门店该商品在本次销售范围内的销售额。调货数量采用混合口径：当近三月+本月迄今平均日销 = 0、库存数量 > 0 且非缺货时，直接按库存数量全调；其它商品沿用系统已有建议调出数量。", "这个表适合门店运营或区域负责人按门店销售贡献度安排调货顺序。它不是简单复制建议调货清单，而是额外引入了销售额排名口径。"],
