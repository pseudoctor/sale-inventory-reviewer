...
def _build_usage_guide_frame() -> pd.DataFrame:
    rows = [
        ...
        ["门店销量排名调货汇总", "该表按近三个月完整自然月 + 本月迄今这一窗口汇总销售额，再按门店销售额降序排名，并列出有调货动作的商品。表中的“门店销售额总计(开始日期至结束日期)”表示该门店在该窗口内的总销售额；“商品销售额(开始日期至结束日期)”表示该门店该商品在同一窗口内的销售额；“商品单价”优先按当前门店该商品在同一窗口内的 商品销售额 / 商品销售数量 计算，若当前门店窗口内销量为 0，则回退到其他门店该商品的平均单价；“库存金额” = 商品单价 * 库存数量。调货数量采用混合口径：当近三月+本月迄今平均日销 = 0、库存数量 > 0 且非缺货时，直接按库存数量全调；其它商品沿用系统已有建议调出数量。", "这个表适合门店运营或区域负责人按近三个月至今的销售贡献度安排调货顺序，并结合库存金额评估压货价值。它不是简单复制建议调货清单，而是额外引入了带时间区间的销售额排名、单价和金额口径。"],
