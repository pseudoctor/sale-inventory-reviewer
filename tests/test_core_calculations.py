...
class CoreCalculationsTest(unittest.TestCase):
    ...
    def test_product_code_catalog_sheet_contains_source_status(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(catalog.columns[:3].tolist(), ["商品条码", "商品编码", "商品名称"])
            barcode_by_code = dict(zip(catalog["商品编码"], catalog["商品条码"].astype(str)))
            name_by_code = dict(zip(catalog["商品编码"], catalog["商品名称"]))
            status_by_code = dict(zip(catalog["商品编码"], catalog["来源状态"]))
            self.assertEqual(barcode_by_code["P1"], "6901")
            self.assertEqual(barcode_by_code["P2"], "6902")
            self.assertEqual(barcode_by_code["P3"], "6903")
            self.assertEqual(name_by_code["P1"], "销售SKU1")
            self.assertEqual(name_by_code["P2"], "仅销售SKU")
            self.assertEqual(name_by_code["P3"], "仅库存SKU")
